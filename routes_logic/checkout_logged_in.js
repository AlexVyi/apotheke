...
var OrderS = require('../models/order-model-logged-in');
...
module.exports = {
  ...
  getCheckout : function (req,res,next) {
    ...
    if(!req.session.cart){//if no cart redirect the user to main
    ...
    if(req.session.cart.constructor === Object && Object.keys(req.session.cart.items).length === 0){//verify if cart is empty and don't let the user move forward to checkout
      logger.info('Tried to go to checkout with empty cart.Who would do such a thing?',
        {headers:req.headers},{method:req.method}
      );
      return res.redirect('/')
    }
    ...
    logger.info('errMsg were triggered, from above gateway.clientToken.generate function, in checkout_logged_in.Must see why', errMsg);

    gateway.clientToken.generate({}, function (err, response) {

      logger.info('In this point the gateway was accessed from checkout logged in',
        {headers:req.headers},
        {method:req.method}
      );

      if(err){
        logger.info('In this point the gateway was accessed from checkout logged in and we have an ERROR',err);
        return next(err)
      }
  ...
  postCheckout: function (req,res,next) {
    ...
    function formatErrors(errors) {
      ...
      for (var i in errors) {//error fn from brain_tree
    ...
    req.sanitizeBody("first_name").trim();//express validator.see npm for details
    ...
    req.checkBody("company").optional({checkFalsy:true}).isAlpha();
    ...
    if (errors) {
      ...
      logger.info("Errors from checkout logged in: " + keys )
      ...
      gateway.transaction.sale({
        ...
        function (error, result) {//see brain_tree docs
          if (result.success || result.transaction) {
            ...
            var order = new OrderS({//if you add something here make sure to add it in the model too.
              user: req.user,
              cart: cart,
              ...
              payment: req.body.payed_with_card_loggedIn

            });
            logger.info(
              'User\'s id accessing the store is : ',
              {user:req.user._id},
              'Card transaction id is : ',
            ...
            order.save(function (err, user) {
              ...
              if (err) {//if err send the user back to shop and display in the view, bellow error, button towards the shop.keep the actual session alive
                logger.info('Error when trying to save the order from checkout logged in : ' ,
                  err,
                  {method:req.method}
                );
                var cart = new Cart(req.session.cart);
              ...
              logger.info('Post checkout logged in and order is saved :' ,
                'User id is : ',
                {user:req.user._id},
                {method:req.method}
              );
            ...
            var transform18 = {//added last this is why the number 18
              '<>': 'ul', 'html': function () {//see http://www.json2html.com/ for details if u forget
            ...
            if(data.company) {
              var mailOptions = {
                ...
                '<h3>Acest mail reprezinta confirmarea comenzii.</h3>' + html + html16 + html17 + html1 + html2 + html3 + html19+html20+html21+ html4 + html5 + html6 + html7 + html8 + html9 + html10 + html11 + html22+ html23 + html24 + html12 + html13 + html14 + html15 + html18 +'<h5>Curier: Urgent Cargus</h5>' + '<h5>Pentru informatii suplimentare sau orice alte situatii apelati gratuit: 0800 41 02 02.L-V 08:00-16:00</h5>'
              ...
              var mailOptions = {
                ...
                '<h3>Acest mail reprezinta confirmarea comenzii.</h3>' + html + html16 + html17 + html1 + html2 + html3 + html19 + html20 + html21 + html4 + html5 + html6 + html7 + html18 +'<h5>Curier: Urgent Cargus</h5>' + '<h5>Pentru informatii suplimentare sau orice alte situatii apelati gratuit: 0800 41 02 02. L-V 8:00-16:00</h5>'
            ...
            smtpTransport.sendMail(mailOptions, function (err, result) {
              if (err) {
                logger.error('Post checkout with card and failed to send email : ' +  err)
                return next('route')
              }
              logger.info('We successfully sent the email with the order from checkout logged in. ',
                'User id is : ',
                {user:req.user._id},
                {method:req.method},
                {body:req.body}
            ...
            logger.info('Post checkout logged in  and failed to process card : ', transactionErrors)
